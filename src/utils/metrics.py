...
def calc_kl_penalty(
    ...
    '''
    Computes the KL divergence between the logits and the reference logits, scaled by the penalty function. 
    This is used to stop the learned policy from diverging too much from the original reference model's policy.

    Args:
        logits: The logits of the generated samples (under the new model).
        ref_logits: The logits of the generated samples (under the reference model).
        kl_coef: The coefficient of the KL penalty, which weights its contribution to the overall objective function.
        prefix_len: The length of the prefix to ignore when computing the KL divergence.
...
def calc_entropy_bonus(
    ...
    Args:
        logits: the logits of the tokens generated by the model.
        ent_coef: the coefficient for the entropy loss, which weights its contribution to the overall objective function.
        prefix_len: the length of the prefix to ignore when computing entropy.
    '''
    log_p = t.log_softmax(logits[:, prefix_len-1:-1, :], dim=-1)
...
def calc_clipped_surrogate_objective(
    ...
    Args:
        logprobs: the logprobs of the action taken by the agent, according to the new policy
        mb_logprobs: logprobs of the actions taken in the sampled minibatch (according to the old policy)
        mb_advantages: advantages calculated from the sampled minibatch
        clip_coef: amount of clipping in the PPO algorithm
        eps: used to add to std dev of mb_advantages when normalizing (to avoid dividing by zero)
    '''
...
def calc_greedy_clipped_surrogate_objective(
    ...
    '''
    Return the clipped surrogate objective, calculated using the greedy advantages, suitable for maximisation with gradient ascent.

    Args:
        logprobs: the logprobs of the action taken by the agent, according to the new policy
        mb_logprobs: logprobs of the actions taken in the sampled minibatch (according to the old policy)
        mb_advantages: advantages calculated from the sampled minibatch
        mb_greedy_advantages: greedy advantages calculated from the sampled minibatch
        clip_coef: amount of clipping in the PPO algorithm
        eps: used to add to std dev of mb_advantages when normalizing (to avoid dividing by zero)
    '''
    assert logprobs.shape == mb_logprobs.shape == mb_advantages.shape,\
